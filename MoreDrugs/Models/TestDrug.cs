...
namespace MoreDrugs.Models
{
    ...
    public class TestDrug : GrabbableObject
    {
        ...
        private static void __rpc_handler_1607080184(NetworkBehaviour target, FastBufferReader reader, __RpcParams rpcParams)
        {
            ...
            {
                ((TestDrug)target).__rpc_exec_stage = __RpcExecStage.Server;
                ((TestDrug)target).RunOutOfFuelServerRpc();
                ((TestDrug)target).__rpc_exec_stage = __RpcExecStage.None;
            }
        }
        ...
        private static void __rpc_handler_3625530963(NetworkBehaviour target, FastBufferReader reader, __RpcParams rpcParams)
        {
            ...
            if ((object)networkManager != null && networkManager.IsListening)
            {
                ((TestDrug)target).__rpc_exec_stage = __RpcExecStage.Client;
                ((TestDrug)target).RunOutOfFuelClientRpc();
                ((TestDrug)target).__rpc_exec_stage = __RpcExecStage.None;
            }
        }
